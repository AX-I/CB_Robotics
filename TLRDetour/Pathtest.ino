...
void logic() {
  ...
  char dx = betterPath[pcount][0] - posx;
  char dy = betterPath[pcount][1] - posy;

  if (state == 0) {
    if (dx == 1) tr = true;
    else if (dx == -1) tl = true;
    else if (dy == -1) reverse = true;
  }
  else if (state == 1) {
    if (dy == 1) tl = true;
    else if (dy == -1) tr = true;
    else if (dx == -1) reverse = true;
  }
  else if (state == 2) {
    if (dx == -1) {tr = true;}
    else if (dx == 1) tl = true;
...
void removeLoops () {
  uint8_t n, m;
  uint8_t xt, yt;
  char sbridge[4] = {-1, -1, -1, -1};
  char fbridge[4] = {-1, -1, -1, -1};
  byte bcount = 0;
  ...
  byte i;
  byte bpcount = 0;
  for (i = 0; i < sbridge[0]; i++) {
    betterPath[i][0] = path[i][0];
    betterPath[i][1] = path[i][1];
  }
  bpcount = sbridge[0];
  
  for (i = 0; i < (bcount - 1); i++) {
    for (n = fbridge[i]; n < sbridge[i + 1]; n++) {
      betterPath[bpcount][0] = path[n][0];
      betterPath[bpcount][1] = path[n][1];
    }
  }
  for (i = fbridge[bcount]; i < 40; i++) {
     betterPath[bpcount][0] = path[i][0];
     betterPath[bpcount][1] = path[i][1];
  }
  pcount = 1;
}
